				...
				if(isset($conn) && $conn){
					...
					if($lid != null)
					{
						if(isset($_SESSION['hospital_user']))
						{
							...
							$cmd = "INSERT INTO hospital_request(lid, priority, dtype, quantity, hid, name, request_time) VALUES('$lid', '$priority', '$dtype', '$quantity', '$hid', '$name', NOW());";
							//print_r($cmd);
							$out = mysqli_query($conn, $cmd);
							//$out = null;
							if($out)
		...
		<?php
			...
			echo "<table>";
				...
				echo "<tr>";
					...
					if(isset($_SESSION['h_request']['name'.$j])){
